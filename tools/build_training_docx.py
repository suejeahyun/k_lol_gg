...
def build_document():
    ...
    is_admin = MODE == "admin"
    running_title = "K-LOL.GG | 사이트·카카오톡 기능 안내" if is_admin else "K-LOL.GG | 전체 사용·운영 교육자료"
    ...
    run = kicker.add_run("K-LOL.GG FUNCTION GUIDE" if is_admin else "K-LOL.GG OPERATIONS HANDBOOK")
    set_run_font(run, size=10, color=GOLD, bold=True)

    title = doc.add_paragraph()
    title.alignment = WD_ALIGN_PARAGRAPH.CENTER
    title.paragraph_format.space_after = Pt(8)
    run = title.add_run("K-LOL.GG 사이트·카카오톡 기능 안내" if is_admin else "K-LOL.GG 전체 사용·운영 교육자료")
    ...
    run = meta.add_run("2026-08-26  |  기능 안내서")
    ...
    run = lead.add_run(
        "K-LOL.GG 사이트와 카카오톡봇에서 제공하는 기능, 명령어, 접수번호와 두 시스템의 연결 방식을 정리했습니다."
        if is_admin else
    ...
    run = status.add_run("카카오톡에서 접수 · 사이트에서 확인 및 처리")
    ...
    doc.core_properties.title = "K-LOL.GG 사이트·카카오톡 기능 안내" if is_admin else "K-LOL.GG 전체 사용·운영 교육자료"
    doc.core_properties.subject = "K-LOL.GG 사이트와 카카오톡봇 기능 설명" if is_admin else "사이트와 카카오톡 봇 전체 기능 교육"
