...
- source `d7432dbb`, tag `destruction-modes-v1.1.1`, Vercel `dpl_8EaBdm5tfDU4hR6Pc4R7tjYTr48u` READY·운영 smoke PASS. DB0046 유지, 운영 참가 신청 데이터 변경 없음.
- 계약445·단위1009(+skip1), 격리 DB·74화면·16상호작용·접근성·HTTP 인증 PASS. [검증·배포 근거](./qa-evidence/destruction-modes-v1.1.1-2026-09-25/README.md).
...
- 협곡 기존 규칙 유지. 칼바람 최근 최대100판 Riot 수집·재개, 칼바람/증바람 관리자 확인 전적·감사·일회성 평가와 경매금 연결. 증바람은 운영자 확인 입력이며 Riot 자동 조회가 아니다. 주장 확정 이후 평가 고정.
- 공통 카드 뒤집기·포인트·효과음, 단계별 운영·재시도·결과 정정·MVP 종료. 계약445·단위1008(+skip1), 세 모드 격리 DB 전체 운영, 58화면·9상호작용·인증 HTTP PASS. [검증·사용 순서](./qa-evidence/destruction-modes-2026-09-25/README.md).
- 실제 Riot 참가자100판 수집 및 운영 로그인 후 대회 생성은 미실행. 운영 공개 조회·권한 경계·자산은 확인했다.
...
- 하이라이트의 YouTube iframe을 차단하던 CSP를 필요한 호스트만 허용하도록 수정했다. 신고 영상의 운영 임베드 실제 재생 41초와 목록 이동 후 재생 확인. 원본 링크·모바일 크기 보완. [영상 QA](./qa-evidence/highlight-playback-v1.0.0-2026-09-25/README.md).
- source `dce23db8bbe684f4bbce26b128ad05b011339b0f`, Vercel `dpl_85zMgkbYY9gu5nzyDNXkgsVLFkfc` READY·운영 alias 일치. 전체 435 계약·960 단위, CI 36064234975 관리자 HTTP 인증 매트릭스 PASS. 운영 데이터 쓰기·스키마 변경 없음. 운영 인증 세션 브라우저 검증은 미실행이며 합성 환경 UI와 운영 비인증 경계를 구분했다.

## 2026-09-25 운영·감사 신청 메뉴 — 운영 적용

- 운영·감사에 모임·외출·건의사항·디스코드 초대·전체 신청 바로가기를 추가했다. 기존 목록·상세·모바일 선택과 표시 명칭을 통일했다.
- 기존 서비스·API·ADMIN 권한·DB 상태는 유지한다. 읽기 전용 조사에서 기존 신청 113건과 payload 오류 0건 확인.
- source `a669aaff5c5cd3d7c1b81e23e71284f35ea054c9`, Vercel `dpl_F5DzqsztXpL4bDSt9KvLqLdqxARt` READY·운영 alias 일치. 전체 검사 435 계약·959 단위와 CI 관리자 HTTP 매트릭스 PASS. [QA](./qa-evidence/operations-forms-menu-v1.0.0-2026-09-25/README.md). 실제 운영 관리자 로그인 브라우저 검증은 미실행이다.

## 2026-09-25 랭킹 갱신 — 예약 코드 운영 적용, MMR 전환 확인 대기

- MMR 전용 5분 cron을 기존 서비스에 연결했다. 기존 공식은 최고 관리자의 명시적 재계산 전까지 409로 보호한다. 실제 MMR 점수 전환은 수행하지 않았다.
- 일반 랭킹은 47회·104게임 원본과 일치, 최근 참가자 29명의 개인 통계 API도 일치한다. 기본 내전 참여 10회 안내·전체 참가자 보기·집계 시각을 추가했다. 특정 누락 개인은 이름/링크 확인 대기다.
- source `39677494087e044a03fa8f3af86e73770a7e3993`, Vercel `dpl_3zvcSXir3SpHRVrMYWX1WhiB2SYB` READY·운영 alias 일치. 전체 검사 435 계약·958 단위, 격리 MMR PG/HTTP, 운영 공개 API·브라우저 360px 검증 PASS. [QA](./qa-evidence/ranking-refresh-v1.0.0-2026-09-25/README.md).
...
- 오류·충돌 및 파티 무변경 복구용 최신 양식은 유지한다. 다음 수정은 목록의 상세 명령으로 양식을 받는다.
- source `91524771bb5d1838a01bc17427660e5545e0b71d`, Vercel `dpl_FU8uB7whrUvgNaJqJxtCApXvS7pu` READY, 운영 alias SHA 일치. 12:20 KST health·현황·상세 읽기 PASS. 전체 검사와 DB/경계 98건 PASS.
- [QA](./qa-evidence/kakao-save-overview-v1.0.0-2026-09-24/README.md). 휴대폰 R27 소스·DB 스키마 변경 없음.

## 2026-09-22 R27 로컬 진단 — 설치 확인, 체감 지연 조사 중

- R26 진단도 지연 후 도착했다. 이후 사용자 버전 응답으로 R27 설치를 확인했으며 체감 지연은 남아 있다고 보고했다.
- R27은 진단을 서버 요청 상태·중복 응답 차단과 분리하고 봇버전에도 측정을 표시한다. 입력 정규화와 고정 진단 로그 추가.
- [새 QA](./qa-evidence/kakao-response-performance-v1.0.1-2026-09-22/README.md). 서버·DB 동작 변경 없음. 실제 폰 진단 응답과 RECEIVED/RETURNED 동일 초 로그는 확인했으며 수신 전 대기·후속 전달 지연은 미확정이다.
...
- 전체 검사 433 계약·951 단위(+1 DB skip), DB/경계 97건 PASS. [QA](./qa-evidence/party-copy-refresh-v1.0.0-2026-09-22/README.md). 서버 변경, 휴대폰 재설치 불필요. R26 설치는 19:47 확인, 봇속도 무응답·실제 지연 원인은 미확인.


## 2026-09-22 카카오 응답 속도·내전 정보 — 서버 운영 적용, R26 설치 확인
...
- 일반 R26: 글자 명령 이미지 조회 생략, 봇속도 진단. 19:47 설치 확인, 봇속도 무응답 보고. 30초 지연 해소는 미확인. 사이트 알림 1.0.3 유지.
