...
## 2026-09-22 R27 로컬 진단 — 설치 확인 대기

- R26 설치는 사용자 버전 응답으로 확인했다. 봇속도 무응답은 VM에서 재현되지 않아 원인을 단정하지 않는다.
- R27은 진단을 서버 요청 상태·중복 응답 차단과 분리하고 봇버전에도 측정을 표시한다. 입력 정규화와 고정 진단 로그 추가.
- [새 QA](./qa-evidence/kakao-response-performance-v1.0.1-2026-09-22/README.md). 서버·DB 동작 변경 없음. 실제 폰 응답·지연 개선 확인은 남아 있다.
