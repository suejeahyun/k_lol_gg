...
자동 캡처 요약은 [SUMMARY.json](./SUMMARY.json), 공지는 [DISCORD_NOTICE.md](./DISCORD_NOTICE.md)에 있다. 실제 운영 화면 검증은 Vercel 배포 후 진행해야 한다.

## 다음 패치 추천

1. LP 입력 옆에 Riot 마지막 동기화 시각을 표시해 수동 값과 공식 값의 출처를 구분한다.
2. 운영 배포 후 실제 승인 계정으로 현재·최고 티어 저장과 재로그인 유지 여부를 확인한다.
