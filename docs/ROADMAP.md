...
S00~S03은 로컬 구현 후보로 통합됐으며 S04부터 순서대로 구현한다. 반복되는 전수 검증과 화면 캡처는 S14에서 한 번에 수행하되, 각 슬라이스에서는 컴파일과 변경 범위의 핵심 테스트를 유지한다. 운영 Vercel 전환과 운영 데이터 변경은 이 로드맵의 실행 범위가 아니다.
