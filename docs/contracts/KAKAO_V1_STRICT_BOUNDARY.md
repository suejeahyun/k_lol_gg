...
- 입장 시스템 문구를 감지해 닉네임 변경·구인구직방·디스코드 안내를 서버 호출 없이 순서대로 전송하는 로컬 안내
...
입장 안내는 MessengerBot R이 전달하는 `들어왔습니다` 시스템 문구를 사용한다. 과거 알림 parser가 방 이름을 안정적으로 제공하지 못한 운영 이력 때문에 방 이름을 권한 또는 실행 조건으로 쓰지 않으며, 이 봇 프로필이 구독한 방에서 공통으로 실행한다.
