...
2026-09-22 적용 범위 보충: 이 문서는 **변경 불가 V1 원본의 식별·oracle 경계**다. 현재 제품 동작의 후속 사용자 승인은 [ADR0011 파티 편집·목록](../architecture/0011-kakao-party-editable-copy-flow.md)과 [ADR0012 내전 편집·회원 연결](../architecture/0012-kakao-inhouse-editable-copy-flow.md)을 따른다. 이 승인 범위에는 원본과 다른 동작이 존재하며, 원본 fixture를 고쳐 동일하다고 만들지 않고 별도 후속 검사로 검증한다. 현재 소스·배포·기기 확인 상태는 [STATUS](../STATUS.md)를 따른다.
...
- `사진상태`, `자동공지`, `공지생성`
- 파티 이름 단축 명령 `상세 N 추가 이름`, `상세 N 삭제 이름`
- 파티 번호를 DRAFT로 먼저 예약하고 시작시간·게임정보·주최자 전체 양식으로 활성화하는 2단계 흐름
- `오픈채팅봇`의 `입장시 할 일` 문구를 감지해 닉네임·구인구직방·디스코드 정보를 서로 다른 네 가지 톤 중 하나로 전송하는 로컬 안내
- `V2도움말`, `V2진단`, `V2연동확인`, raw JSON 운영 명령
- HMAC V4 서명, nonce, event ID, installation scope, 5초 단일 전송

확장은 별도 테스트에서 검증한다. **후속 사용자 승인이 없는 변경**은 V1_STRICT 명령의 분류와 정확 응답을 바꿀 수 없고 충돌 시 V1_STRICT를 우선한다. 상단에 연결한 승인 ADR의 파티·내전 편집 정책은 해당 범위의 제품 응답을 대체하되 원본 oracle은 그대로 보존한다. 카카오 스크림 종료도 현재 제품의 승인 변경이며 원본 스크림 기능을 재활성화하는 근거로 이 목록을 사용하지 않는다.

원시 `들어왔습니다` 시스템 문구는 안내를 보내지 않는다. `오픈채팅봇` 표시명과 공백을 제거한 `입장시할일` 문구는 권한 근거가 아니라 중복 안내를 피하기 위한 로컬 필터로만 사용한다. 과거 알림 parser가 방 이름을 안정적으로 제공하지 못한 운영 이력 때문에 방 이름을 실행 조건으로 쓰지 않으며, 이 봇 프로필이 구독한 방에서 공통으로 실행한다.
...
후속 승인 변경 범위 밖에서는 서버 내부 구현이 달라도 동일 입력과 동일 서버 결과에 대한 사용자 관찰 가능 출력이 V1_STRICT와 일치해야 한다.
