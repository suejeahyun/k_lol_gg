...
- 사용자가 주최자를 채운 전체 양식을 보내면 같은 DRAFT를 `IN_PROGRESS`로 전환하고 주최자를 첫 번째 참가자로 함께 저장한다. 빈 시작시간은 현재 KST, 빈 게임정보는 `미입력`으로 기본화하며, 세 항목 모두 빈 무편집 양식은 거절한다. 구형 참가행 양식은 한 명 이상 채우면 종전대로 활성화한다.
- 양식만 생성하고 제출하지 않은 경우 활성 파티가 생긴 것처럼 보이면 안 된다.
- 최초 양식에는 값이 비어 있는 `》시작시간 :`, `》게임정보 :`, `》주최자 :`를 표시하고 참가행을 넣지 않는다. 주최자 메타데이터는 별도 저장·표시하되 최초 활성화 transaction에서 같은 이름을 첫 번째 참가 슬롯에도 저장한다. 이후 메타데이터 수정은 참가 명단을 자동 교체하지 않는다.
