...
var KLOL_V4 = (function () {
  ...
  function logEventMaterial(profileId, logId) {
    return profile(profileId) + "\n" + BOOT_ID + "\n" + trimText(logId);
  }

  function nextEventId(profileId, logId) {
    var stableLogId = trimText(logId);
    if (stableLogId) return "event-log-" + sha256(logEventMaterial(profileId, stableLogId)).substring(0, 32);
    eventCounter += 1;
  ...
  function normalizePartyMemberMutationText(value) {
  ...
  function memberMutationProfile(value) {
    var text = normalizePartyMemberMutationText(value);
    var match = null;
    var name = "";
    var nameLength = 0;
    var prefix = "";
    if (/[\r\n\u2028\u2029]/.test(text) || text.indexOf("//") === 0 || /^\/\s/.test(text)) return null;
    if (text.charAt(0) === "/") text = text.substring(1);
    match = /^(구인상세|상세|내전\s*(?:상세|명단)|스크림\s*(?:상세|명단))\s+(?:\\?#\s*)?(\d{1,2})\s+(추가|추가해|추가하기|추기|등록|참가|삭제|삭제해|삭제하기|삭재|제외|탈퇴)\s+(.+)$/.exec(text);
    if (!match || Number(match[2]) < 1 || Number(match[2]) > 99) return null;
    prefix = String(match[1] || "").replace(/\s/g, "");
    name = String(match[4] || "").replace(/^[ \t]+|[ \t]+$/g, "").replace(/[ \t]+/g, " ");
    nameLength = name.replace(/[\uD800-\uDBFF][\uDC00-\uDFFF]/g, "_").length;
    if (nameLength < 1 || nameLength > 80 || /[\/,，、;；]/.test(name) ||
        /^(?:추가|추기|등록|참가|삭제|삭재|제외|탈퇴)(?:\s|$)/.test(name) ||
        /[\u0000-\u001F\u007F-\u009F\u061C\u200E\u200F\u202A-\u202E\u2066-\u2069]/.test(name)) return null;
    return prefix.indexOf("내전") === 0 ? "FEATURES" : "RECRUIT";
  }
  ...
  function acceptsPublicText(profileId, value) {
    ...
    if (profileId === "RECRUIT") {
      if (memberMutationProfile(value) === "RECRUIT") return true;
    ...
    if (memberMutationProfile(value) === "FEATURES") return true;
  ...
  function publicProfileId(value) {
    var command = canonicalLocalCommand(value);
    var recruit = false;
    var features = false;
    if (!command) return null;
    recruit = acceptsPublicText("RECRUIT", value);
    features = acceptsPublicText("FEATURES", value);
    if (recruit && !features) return "RECRUIT";
    if (features && !recruit) return "FEATURES";
    if (recruit && features) return "FEATURES";
    return null;
  }

  function unifiedLocalReply(text, codeVersion) {
    var command = canonicalLocalCommand(text);
    var reply = null;
    if (command === "봇버전") {
      return "[K-LOL.GG V4 봇 버전]\n프로필: UNIFIED\n버전: " + codeVersion +
        "\nRECRUIT 설치본: " + installationId("RECRUIT") +
        "\nFEATURES 설치본: " + installationId("FEATURES");
    }
    reply = localReply("RECRUIT", text, codeVersion);
    if (reply) return reply;
    return localReply("FEATURES", text, codeVersion);
  }
  ...
  function shouldIgnoreUnified(text, sender) {
    var configuredName = setting("KLOL_V4_BOT_SELF_NAME_UNIFIED");
    if (shouldIgnore("RECRUIT", text, sender) || shouldIgnore("FEATURES", text, sender)) return true;
    return Boolean(configuredName && trimText(sender) === configuredName);
  }
  ...
  return {
    ...
    shouldIgnoreUnified: shouldIgnoreUnified,
    unifiedLocalReply: unifiedLocalReply
  };
}());
