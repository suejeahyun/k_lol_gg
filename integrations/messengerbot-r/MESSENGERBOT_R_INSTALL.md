...
사용자가 확인한 **MessengerBot R 앱 버전은 0.7.29a**다. 이것은 아래 봇 JavaScript 버전과 다르다. 19:47 사용자 로그에서 일반 R26 설치는 확인했으나 봇속도 무응답을 보고했다. 실제 지연 개선은 미확인이다. 로컬 생성된 일반 봇 설치 후보는 **R27** (`KLOL_KAKAO_BOT_V40_R27_2026_09_22`)이며, 교체할 때에는 아래 **비공개 한 파일만** 전체 복사한다.
...
5. 파일 안에서 `KLOL_KAKAO_BOT_V40_R27_2026_09_22`를 검색한다.
...
글자 메시지에서는 SDK 이미지 조회를 건너뛰고 기존 명령 라우터로 전달한다. 실제 사진·빈 메시지 경로는 유지한다. 일반 봇 KLOLMain을 R27로 교체하고 저장·컴파일한 뒤 /봇버전 → 내전상세 1 → 봇속도를 각각 전송한다. 봇속도는 직전 서버 요청의 휴대폰 처리 시간과 HTTP 왕복 시간을 표시하며, 카카오 알림이 콜백에 도착하기 전 대기는 측정하지 못한다. 서버 요청 없는 대화·봇 echo는 직전 측정을 덮지 않는다.

사이트 알림 companion 1.0.3은 교체하지 않는다. 내전 양식의 내전 정보: 항목에는 저티어 내전 / 일반내전 같은 설명을 적을 수 있다. 빈 값은 설명 삭제, 항목이 없는 예전 양식은 저장된 설명 유지다.

R27은 봇속도·봇진단·봇버전을 서버 요청 상태 초기화와 중복 응답 차단 전에 처리하고, 세 명령 모두 버전과 마지막 측정을 표시한다. 공백·전각 슬래시·제로폭 문자를 정리한다. 무응답 시 KLOLMain 로그의 `[KLOL_DIAG] RECEIVED`, `RETURNED`, `REJECTED`, `REPLY_ERROR`로 수신과 응답 호출 결과를 구분한다. RETURNED는 SDK 호출 반환이며 실제 채팅 수신 증거가 아니다. 로그에는 메시지·방·이름·예외 원문을 기록하지 않는다. R26 무응답의 실기기 원인은 아직 확정하지 않았다.
