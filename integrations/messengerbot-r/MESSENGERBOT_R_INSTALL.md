...
5. 설치본 안에 `KLOL_KAKAO_BOT_V41_V3_2026_09_09_R14_INSTALLATION_DEDUPE`가 있는지 검색한다.
...
8. 같은 방을 구독하는 다른 MessengerBot R 봇·스크립트·프로필을 모두 중지한다. R14 한 개만 활성 상태여야 한다.
9. 카카오톡 방에서 `/봇버전`과 `봇버전`을 각각 보내 `KLOL_KAKAO_BOT_V41_V3_2026_09_09_R14_INSTALLATION_DEDUPE`, 설치본 ID, 키 ID가 동일하게 출력되는지 확인한다.
...
| 확인 결과 | 판정 |
| --- | --- |
| 91줄 + R14 버전 + 아래 SHA-256 일치 | 설정 분리형 휴대폰 설치본 |
...
| --- | --- | --- | ---: | ---: | --- |
| MessengerBot R 설정 분리형 | `KLOL_KAKAO_BOT_V41_MESSENGERBOT_R.js` | `KLOL_KAKAO_BOT_V41_V3_2026_09_09_R14_INSTALLATION_DEDUPE` | 91 | 75,915 bytes / 64,358자 | `242dac7363414599b6716a672107c98fcf63040d80c3164bf619ffa7c589bdca` |
| 개발·검토용 | `KLOL_KAKAO_BOT_V41_V2_COMPLETE.js` | `KLOL_KAKAO_BOT_V41_V3_2026_09_09_R14_INSTALLATION_DEDUPE` | 2,777 | 133,623 bytes / 121,928자 | `ef1a0a9ad3ba3c86b7f715a406c219e256b8e251d8e1211e31a9ee99b355e28e` |

해시, 크기 또는 끝 표시가 다르면 다른 버전이거나 전송 과정에서 변형된 파일이다.

설치본의 LF가 모두 CRLF로 변환되는 보수적 계산은 64,448자로 65,535자 이하다. 전송 도구가 다른 문자를 추가하지 않게 파일 자체를 그대로 설치한다.
...
`ROOM_BINDING_REQUIRED`이면 sender나 환경변수를 추가하지 말고 해당 방의 `/V2연동확인` 결과 전체를 관리자에게 전달한다. R14는 MessengerBot R 0.7.34a 이상의 `channelId`를 방 fingerprint의 우선 근거로 사용하므로 같은 방에서는 발신자와 방 제목 변경에 관계없이 같은 fingerprint가 나온다. `channelId`가 없고 `room=sender`인 손상된 구형 알림 callback은 권한 요청을 안전하게 거부한다. 여러 휴대폰/봇 설치본은 서로 다른 로컬 방 fingerprint를 만들 수 있으며 관리자가 일회용 코드로 같은 canonical 방을 지정해야 한다.

R14 휴대폰 설치본은 `/봇버전`과 `/V2연동확인`에 공개 설치본 ID와 키 ID를 표시한다. V3 서명은 이 두 값과 봇 버전·메시지 전달 ID를 함께 묶는다. 같은 카카오 메시지가 복수 설치본에서 전송돼도 canonical 방+전달 ID 멱등성으로 상태 변경은 한 번만 수행한다. 키 교체 시 서버의 current/previous ID를 먼저 설정한 뒤 휴대폰 키와 ID를 current로 바꾸며, 이전 키 ID로 고정된 설치본은 새 current ID로 한 번 승격된 뒤 되돌아갈 수 없다.

MessengerBot R 앱 자체가 0.7.34a 미만이면 먼저 최신 안정 버전으로 업데이트해야 한다. migration `0031_brainy_taskmaster.sql`, `0032_bent_ultimates.sql`, 대응 서버, R14 휴대폰 설치본을 하나의 점검 시간에 적용하고 혼용 중에는 모집 변경을 중지한다. 정상 요청은 Vercel `KAKAO_WEBHOOK_ALLOWED_ROOMS`와 방 fingerprint를 비교하지 않으며, 미등록 방은 `ROOM_BINDING_REQUIRED` 후 DB pairing만 사용한다. 이 저장소 작업은 실제 설치나 배포를 수행하지 않았다.
