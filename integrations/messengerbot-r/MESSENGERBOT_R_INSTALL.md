...
5. 설치본 안에 `KLOL_KAKAO_BOT_V41_V2_2026_09_09_R9_DB_ROOM_BINDINGS`가 있는지 검색한다.
...
8. 카카오톡 방에서 `/봇버전`과 `봇버전`을 각각 보내 `KLOL_KAKAO_BOT_V41_V2_2026_09_09_R9_DB_ROOM_BINDINGS`가 출력되는지 확인한다.
...
| 확인 결과 | 판정 |
| --- | --- |
| 468줄 + R9 버전 + 아래 SHA-256 일치 | 설정 분리형 휴대폰 설치본 |
...
| --- | --- | --- | ---: | ---: | --- |
| MessengerBot R 설정 분리형 | `KLOL_KAKAO_BOT_V41_MESSENGERBOT_R.js` | `KLOL_KAKAO_BOT_V41_V2_2026_09_09_R9_DB_ROOM_BINDINGS` | 468 | 75,981 bytes / 64,688자 | `9717eff2280c58fcb10c65784f8227d3b4a8dc28504793a878804a980a856321` |
| 개발·검토용 | `KLOL_KAKAO_BOT_V41_V2_COMPLETE.js` | `KLOL_KAKAO_BOT_V41_V2_2026_09_09_R9_DB_ROOM_BINDINGS` | 2,671 | 127,142 bytes / 115,715자 | `2de1e4e3d9d66a4387b311c5e6e403fae5b09dd4b871e558077577f638bfbd73` |

해시, 크기 또는 끝 표시가 다르면 다른 버전이거나 전송 과정에서 변형된 파일이다.

설치본의 LF가 모두 CRLF로 변환되는 보수적 계산은 65,155자로 65,535자 이하다. 전송 도구가 다른 문자를 추가하지 않게 파일 자체를 그대로 설치한다.
...
`ROOM_BINDING_REQUIRED`이면 sender나 환경변수를 추가하지 말고 해당 방의 `/V2연동확인` 결과를 관리자에게 전달한다. 여러 휴대폰/봇 설치본은 서로 다른 로컬 방 fingerprint를 만들 수 있으며, 방 이름이 같아도 자동 병합하지 않는다. 관리자가 일회용 코드로 같은 canonical 방을 지정해야 한다.

R9 서버는 V2 서명 material에 공개 `botInstallationId`를 포함한다. migration `0031_brainy_taskmaster.sql`, R9 서버, R9 휴대폰 설치본을 하나의 점검 시간에 적용하고 혼용 중에는 모집 변경을 중지한다. 기존 환경변수 방은 최초 요청에서 bootstrap binding으로 비파괴 이관되지만 신규 방은 DB 페어링만 사용한다. 이 저장소 작업은 실제 설치나 배포를 수행하지 않았다.
