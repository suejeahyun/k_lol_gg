...
5. 설치본 안에 `KLOL_KAKAO_BOT_V41_V2_2026_09_09_R8_CONTROLLER_AUTH_SYNC`가 있는지 검색한다.
...
8. 카카오톡 방에서 `/봇버전`과 `봇버전`을 각각 보내 `KLOL_KAKAO_BOT_V41_V2_2026_09_09_R8_CONTROLLER_AUTH_SYNC`가 출력되는지 확인한다.
...
| --- | --- | --- | ---: | ---: | --- |
| MessengerBot R 설정 분리형 | `KLOL_KAKAO_BOT_V41_MESSENGERBOT_R.js` | `KLOL_KAKAO_BOT_V41_V2_2026_09_09_R8_CONTROLLER_AUTH_SYNC` | 966 | 75,669 bytes / 64,489자 | `f0f5f5d950c75b93a573a8167ae48ea02e812936084dc7faf3096063ad06839d` |
| 개발·검토용 | `KLOL_KAKAO_BOT_V41_V2_COMPLETE.js` | `KLOL_KAKAO_BOT_V41_V2_2026_09_09_R8_CONTROLLER_AUTH_SYNC` | 2,653 | 125,873 bytes / 114,559자 | `97647b713a9b2a27760a9440ee187f843dbc71a5a221384515ac15acf62e918d` |

해시, 크기 또는 끝 표시가 다르면 다른 버전이거나 전송 과정에서 변형된 파일이다.

설치본의 LF가 모두 CRLF로 변환되는 보수적 계산은 65,454자로 65,535자 이하다. 다만 여유가 81자뿐이므로 전송 도구가 다른 문자를 추가하지 않게 파일 자체를 그대로 설치한다.
...
모집 생성·조회와 스크림 참가, 내전 전체 양식 제출·현황 조회, 외출·휴식·친구·정모·건의 양식 제출은 같은 허용 방의 공개 capability다. 모집 동기화·종료·취소·재개·확정·완료는 생성자, 참가한 상대 팀장 또는 sender allowlist 운영자만 가능하다. 내전 강제 취소·예약 공지·이미지 접수·raw V2 JSON은 trusted sender가 필요하고, 운영 양식 검토·삭제는 웹 ADMIN 세션만 가능하다. `/` 유무는 이 권한 판정에 영향을 주지 않는다.

`KAKAO_ROOM_FORBIDDEN`이면 sender를 추가하지 말고 해당 방의 `/V2연동확인` 결과를 관리자에게 전달해 `KAKAO_WEBHOOK_ALLOWED_ROOMS`와 정확히 대조한다.

R8 서버는 모집 요청의 서명된 `COMPAT_V1`/`RAW_V2` 출처 표식을 필수로 검사하므로 R7 휴대폰 코드와 호환되지 않는다. migration `0030_secret_darwin.sql`, R8 서버, R8 휴대폰 설치본을 하나의 점검 시간에 교체하고 혼용 중에는 모집 변경을 중지한다. 이 저장소 작업은 실제 설치나 배포를 수행하지 않았다.
