...
모집 생성·조회와 스크림 참가, 외출·친구·정모·건의 양식 제출은 같은 허용 방의 공개 capability다. 동기화·종료·취소·재개·확정·완료는 생성자, 참가한 상대 팀장 또는 sender allowlist 운영자만 가능하다. 내전 authoritative 신청·예약 공지·이미지 접수는 trusted sender capability가 필요하고, 운영 양식 검토·삭제는 웹 ADMIN 세션만 가능하다. `/` 유무는 이 권한 판정에 영향을 주지 않는다.
