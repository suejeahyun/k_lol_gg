...
5. 설치본 안에 `KLOL_KAKAO_BOT_V41_V2_2026_09_09_R11_ROOM_CALLBACK_TRACE`가 있는지 검색한다.
...
8. 카카오톡 방에서 `/봇버전`과 `봇버전`을 각각 보내 `KLOL_KAKAO_BOT_V41_V2_2026_09_09_R11_ROOM_CALLBACK_TRACE`와 동일한 설치 지문이 출력되는지 확인한다.
...
| --- | --- | --- | ---: | ---: | --- |
| MessengerBot R 설정 분리형 | `KLOL_KAKAO_BOT_V41_MESSENGERBOT_R.js` | `KLOL_KAKAO_BOT_V41_V2_2026_09_09_R11_ROOM_CALLBACK_TRACE` | 219 | 76,377 bytes / 65,046자 | `71bda4b0f3149caf2ba9ae6edb48f0268a0bd658acca992ec709b551f9fe3de4` |
| 개발·검토용 | `KLOL_KAKAO_BOT_V41_V2_COMPLETE.js` | `KLOL_KAKAO_BOT_V41_V2_2026_09_09_R11_ROOM_CALLBACK_TRACE` | 2,696 | 128,334 bytes / 116,869자 | `237cf808c91aec9d2657772c751227d96b70cdc3a819c25fc244e7e9a8d57efe` |

해시, 크기 또는 끝 표시가 다르면 다른 버전이거나 전송 과정에서 변형된 파일이다.

설치본의 LF가 모두 CRLF로 변환되는 보수적 계산은 65,264자로 65,535자 이하다. 전송 도구가 다른 문자를 추가하지 않게 파일 자체를 그대로 설치한다.
...
`ROOM_BINDING_REQUIRED`이면 sender나 환경변수를 추가하지 말고 해당 방의 `/V2연동확인` 결과 전체를 관리자에게 전달한다. 같은 설치본·같은 정규화 방 문자열은 발신자가 달라도 같은 방 fingerprint를 만든다. R11은 NFKC 정규화와 불안정한 zero-width 문자 제거 후 실제 HMAC 입력의 UTF-16 코드 단위를 함께 표시한다. 여러 휴대폰/봇 설치본은 서로 다른 로컬 방 fingerprint를 만들 수 있으며, 방 이름이 같아도 자동 병합하지 않는다. 관리자가 일회용 코드로 같은 canonical 방을 지정해야 한다.

R11 휴대폰 설치본은 `/봇버전`과 `/V2연동확인`에 공개 `botInstallationId`를 표시하며, 서버 서명 material에도 같은 값을 포함한다. 두 응답의 설치 지문이 다르면 표시 봇 이름과 관계없이 서로 다른 identity secret을 사용하는 실행본이다. 설치 지문이 같고 방 지문이 다르면 `방입력 UTF-16` 값을 비교해 MessengerBot R 콜백 차이를 확인한다. migration `0031_brainy_taskmaster.sql`, 대응 서버, R11 휴대폰 설치본을 하나의 점검 시간에 적용하고 혼용 중에는 모집 변경을 중지한다. 기존 환경변수 방은 최초 요청에서 bootstrap binding으로 비파괴 이관되지만 신규 방은 DB 페어링만 사용한다. 이 저장소 작업은 실제 설치나 배포를 수행하지 않았다.
