...
현재 공개 소스 버전은 `KLOL_SITE_NOTICE_COMPANION_1.0.1`이다. 일반 R25 봇의 실기기 `봇버전`·구인현황·내전현황 성공 로그는 확인했지만, 별도 companion 설치·백그라운드 충원 수신은 아직 확인하지 않았다.
...
| 입력 | 확인 내용 |
|---|---|
| `사이트알림버전` | `KLOL_SITE_NOTICE_COMPANION_1.0.1` 버전 |
| `사이트알림상태` | 버전, 휴대폰 알림 켜짐/꺼짐, 수신 세션 등록됨/등록 필요 |

두 명령은 휴대폰 로컬 상태만 조회한다. OFF에서도 확인할 수 있으며, 서버 연결 검사·큐 조회·충원 안내 발송·등록코드 소모·수신 세션 변경을 하지 않는다. `등록됨`은 이 봇이 보관 중인 세션이 있다는 뜻으로, 상태를 조회한 방이 수신 대상이라는 뜻은 아니다. 등록 성공 안내 역시 실제 충원 메시지 수신 확인과 구분한다. 개인방·다른 앱·일반 봇 명령에는 응답하지 않는다.
