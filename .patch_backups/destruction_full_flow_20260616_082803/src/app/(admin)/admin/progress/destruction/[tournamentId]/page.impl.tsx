...
function CaptainPreferenceApplyList({
  applies,
}: {
  applies: CaptainPreferenceApply[];
}) {
  const preferredCount = applies.filter((apply) => apply.isCaptain).length;
  const notPreferredCount = applies.length - preferredCount;

  return (
    <div className="admin-form" style={{ marginBottom: 16 }}>
      <div className="admin-page__header">
        <div>
          <h3 className="admin-event-section-title">팀장 선호 현황</h3>
          <p className="admin-page__description">
            유저 참가 신청 시 선택한 팀장 선호/비선호를 확인합니다. 최종 팀장은 아래 팀장/팀 입력에서 관리자가 직접 지정합니다.
          </p>
        </div>

        <div className="admin-event-detail-actions">
          <span className="chip-button">선호 {preferredCount}명</span>
          <span className="chip-button">비선호 {notPreferredCount}명</span>
        </div>
      </div>
      ...
      {applies.length === 0 ? (
        ...
        <div className="admin-event-participant-list">
          {applies.map((apply) => (
            <div key={apply.id} className="admin-event-participant-row">
              <span>
                {apply.player.name} · {apply.player.nickname}#{apply.player.tag}
              </span>
              <span>{apply.mainPosition}</span>
              <span>
                {apply.subPositions.length > 0 ? apply.subPositions.join(", ") : "부포지션 없음"}
              </span>
              <span>{apply.player.currentTier ?? "현재티어 없음"}</span>
              <span>{apply.player.peakTier ?? "최고티어 없음"}</span>
              <strong>{apply.isCaptain ? "팀장 선호" : "팀장 비선호"}</strong>
...
export default async function AdminDestructionTournamentDetailPage({
  ...
  const captainCandidates: CaptainCandidate[] = tournament.participationApplies.map((apply) => ({
    id: apply.player.id,
    ...
    captainPreference: apply.isCaptain ? "PREFERRED" : "NOT_PREFERRED",
  ...
  return (
    <main className="admin-page">
      ...
      >
        <CaptainPreferenceApplyList applies={tournament.participationApplies} />
