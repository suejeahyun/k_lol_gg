...
test("public Riot profile separates unlinked, connected, stale, temporary and rate-limited states", () => {
  assert.equal(resolvePublicRiotProfileState({ playerFound: true, linked: false, summary: null, latestSync: null, now }).kind, "UNLINKED");
  assert.deepEqual(resolvePublicRiotProfileState({ playerFound: true, linked: true, summary: null, latestSync: null, now }), { kind: "PENDING_SYNC", connectionState: "CONNECTED" });
