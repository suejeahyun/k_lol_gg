...
const contract = JSON.parse(await readFile(new URL("./fixtures/kakao-v4-v1-compatibility-contract.json", import.meta.url), "utf8")) as {
  inhouse: { modeSelectorReply: string; invalidModeSelectorReply: string; riftActivationTemplate: string; aramActivationTemplate: string; riftTemplate: string; aramTemplate: string; multiRoundStatusReply: string };
...
test("Phase 3-A: 내전 모드 선택과 협곡·칼바람 활성화 양식을 정확히 보존한다", async () => {
  const state = harness();
  ...
  assert.equal((await reply(state.service, envelope("FEATURES", "내전구인 협곡 2026-09-09 21:30 #2 10명", 3))).reply, contract.inhouse.riftActivationTemplate);
  assert.equal((await reply(state.service, envelope("FEATURES", "내전구인 칼바람 2026-09-09 21:30 #3 10명", 4))).reply, contract.inhouse.aramActivationTemplate);
