...
test("public destruction detail keeps owner application, vote and explicit unavailable/error states", () => {
  ...
  const mutation = source("../src/components/competitions/destruction/use-destruction-mutation.ts");
...
test("administrator destruction workspaces expose the full staged lifecycle", () => {
  ...
  const workflow = source("../src/modules/competitions/destruction/workflow.ts");
  ...
  assert.match(actions, /aria-live="polite"/);
  assert.match(detail, /parseDestructionAdminDetailView/);
  assert.match(detail, /auction-live/);
  assert.match(source("../src/components/competitions/destruction/live-status.tsx"), /8_000/);
});
