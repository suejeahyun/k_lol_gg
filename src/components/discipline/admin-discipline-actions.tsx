...
export function AdminDisciplineActions({ record }: { record: AdminDisciplineRecordDto }) {
  ...
  return <section className={`${styles.panel} ${styles.form}`}><h2>관리 작업</h2><label className={styles.field}><span>사유</span><textarea rows={4} maxLength={1000} value={reason} onChange={(event) => setReason(event.target.value)} /></label><label className={styles.field}><span>내부 메모</span><textarea rows={3} maxLength={2000} value={note} onChange={(event) => setNote(event.target.value)} /></label><div className={styles.actions}><button className={styles.button} disabled={busy} onClick={() => mutate(`/api/admin/discipline-records/${record.id}`, "PATCH", record.revision, { reason, internalNote: note || null })}>내용 저장</button>{record.active ? <button className={styles.link} disabled={busy} onClick={() => mutate(`/api/admin/discipline-records/${record.id}`, "DELETE", record.revision, { reason: "관리자 취소" })}>기록 취소</button> : null}</div>
    {record.task?.status === "PENDING_REVIEW" ? <><label className={styles.field}><span>검토 메모</span><textarea rows={3} maxLength={1000} value={reviewNote} onChange={(event) => setReviewNote(event.target.value)} /></label><div className={styles.actions}><button className={styles.button} disabled={busy} onClick={() => mutate(`/api/admin/discipline-tasks/${record.task!.id}/review`, "PATCH", record.task!.revision, { decision: "APPROVE", reviewNote })}>과제 승인</button><button className={styles.link} disabled={busy || !reviewNote.trim()} onClick={() => mutate(`/api/admin/discipline-tasks/${record.task!.id}/review`, "PATCH", record.task!.revision, { decision: "REJECT", reviewNote })}>보완 요청</button><button className={styles.link} disabled={busy || !reviewNote.trim()} onClick={() => mutate(`/api/admin/discipline-tasks/${record.task!.id}/review`, "PATCH", record.task!.revision, { decision: "CANCEL", reviewNote })}>과제 취소</button></div></> : null}
