...
export async function AdminOperationFormList({ selectedType, selectedStatus }: { selectedType?: string; selectedStatus?: string }) {
  ...
  return <main className={styles.page}>
    ...
    })}</tbody></table></div> : result.state === "ready" ? <section className={styles.state}><h2>조건에 맞는 신청이 없습니다.</h2><p>필터를 바꾸거나 새 접수를 기다려 주세요.</p></section> : result.state === "unavailable" ? <section className={styles.state} role="status"><h2>저장소 연결을 준비 중입니다.</h2><p>연결이 완료되면 접수 목록이 여기에 표시됩니다.</p></section> : <section className={styles.state} role="alert"><h2>신청서를 불러오지 못했습니다.</h2><p>잠시 후 다시 시도해 주세요.</p></section>}
  </main>;
}
