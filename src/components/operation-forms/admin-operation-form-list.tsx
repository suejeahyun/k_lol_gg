...
const statusLabels = Object.freeze({ PENDING: "대기", IN_REVIEW: "검토 중", COMPLETED: "완료", REJECTED: "반려", CANCELLED: "취소" });
...
export async function AdminOperationFormList({ selectedType, selectedStatus }: { selectedType?: string; selectedStatus?: string }) {
  ...
  return <main className={styles.page}>
    <header className={styles.header}><div><span className={styles.eyebrow}>ADMIN · OPERATION FORMS</span><h1>운영 신청서</h1><p>카카오에서 접수된 네 가지 신청을 분류하고 상태와 관리 메모를 처리합니다.</p></div><Link className={styles.link} href="/admin/kakao">Kakao 관리</Link></header>
    ...
    <form className={styles.filters} method="get" action={formType ? `/admin/operation-forms/${formType}` : "/admin/operation-forms"}>
      ...
      <label>상태<select name="status" defaultValue={status ?? ""}><option value="">전체 상태</option>{OPERATION_FORM_STATUSES.map((item) => <option key={item} value={item}>{statusLabels[item]}</option>)}</select></label><button type="submit">필터 적용</button>
    </form>
    {result.state === "ready" && result.data.items.length ? <div className={styles.tableWrap}><table className={styles.table}><thead><tr><th>유형</th><th>신청자</th><th>상태</th><th>접수</th><th /></tr></thead><tbody>{result.data.items.map((form) => {
      ...
      return <tr key={form.id}><td>{typeLabels[form.formType]}</td><td>{applicant}</td><td><span className={styles.badge}>{statusLabels[form.status]}</span></td><td>{new Intl.DateTimeFormat("ko-KR", { dateStyle: "medium", timeStyle: "short", timeZone: "Asia/Seoul" }).format(new Date(form.submittedAt))}</td><td><Link href={`/admin/operation-forms/${form.formType}/${form.id}`}>상세</Link></td></tr>;
    })}</tbody></table></div> : result.state === "ready" ? <section className={styles.state}><h2>조건에 맞는 신청이 없습니다.</h2><p>샘플 신청서는 표시하지 않습니다.</p></section> : result.state === "unavailable" ? <section className={styles.state} role="status"><h2>저장소 연결을 준비 중입니다.</h2><p>연결이 완료되면 접수 목록이 여기에 표시됩니다.</p></section> : <section className={styles.state} role="alert"><h2>신청서를 불러오지 못했습니다.</h2><p>잠시 후 다시 시도해 주세요.</p></section>}
  </main>;
}
