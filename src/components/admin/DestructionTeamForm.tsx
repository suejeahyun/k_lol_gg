...
function getStatusMessage(activeCount: number, requiredCaptainCount: number, selectedCount: number) {
  if (activeCount === 0) return "참가 신청을 먼저 받아야 합니다.";
  if (activeCount % 5 !== 0) return `참가자 수가 5의 배수가 아닙니다. 현재 ${activeCount}명입니다.`;
  if (selectedCount < requiredCaptainCount) return `팀장 ${requiredCaptainCount - selectedCount}명을 더 지정해야 합니다.`;
  if (selectedCount > requiredCaptainCount) return `팀장이 ${selectedCount - requiredCaptainCount}명 초과되었습니다.`;
...
export default function DestructionTeamForm({
  ...
  const [teamNames, setTeamNames] = useState<Record<number, string>>(() => {
    ...
    for (const application of applications) {
      ...
      value[application.playerId] = existing?.name ?? toDefaultTeamName(application);
    }
    return value;
  });
  const [points, setPoints] = useState<Record<number, string>>(() => {
    ...
    for (const application of applications) {
      const existing = existingTeamByCaptainId.get(application.playerId);
      value[application.playerId] = String(existing?.initialAuctionPoints ?? 100);
    }
    return value;
  });
  ...
  const requiredCaptainCount = getRequiredCaptainCount(activeApplications.length);
  const preferredCount = activeApplications.filter((application) => application.isCaptain).length;
  const selectedCount = selectedCaptainIds.length;
  const canCreateTeams =
    requiredCaptainCount > 0 && selectedCount === requiredCaptainCount && !hasMatches;

  const applicationByPlayerId = useMemo(() => {
    return new Map(activeApplications.map((application) => [application.playerId, application]));
  }, [activeApplications]);

  const selectedApplications = selectedCaptainIds
  ...
  const toggleCaptain = (playerId: number) => {
    ...
    setSelectedCaptainIds((prev) => {
      ...
      if (requiredCaptainCount > 0 && prev.length >= requiredCaptainCount) {
        setError(`팀장은 정확히 ${requiredCaptainCount}명까지만 지정할 수 있습니다.`);
        return prev;
      }
      setError("");
  ...
  const handleSubmit = async () => {
    ...
    if (activeApplications.length === 0) {
      setError("참가 신청자가 없습니다. 유저 페이지에서 참가 신청을 먼저 받아주세요.");
      return;
    }

    if (activeApplications.length % 5 !== 0) {
      setError(`참가자 수가 5의 배수가 아닙니다. 현재 ${activeApplications.length}명입니다.`);
      return;
    }
    ...
    const teams = selectedCaptainIds.map((captainId) => {
      ...
      return {
        name: (teamNames[captainId] ?? "").trim() || (application ? toDefaultTeamName(application) : "팀"),
        captainId,
        captainPosition: application?.mainPosition,
        initialAuctionPoints: Number(points[captainId] ?? 0),
      };
    });
    ...
    try {
      const res = await fetch(`/api/destruction-tournaments/${tournamentId}/teams`, {
        method: "POST",
        headers: {
  ...
  return (
    <div className="destruction-team-form destruction-admin-panel-wide">
      ...
      <div className="destruction-captain-summary">
        ...
        <div className="admin-event-detail-card">
          <span>필요 팀장</span>
          <strong>{requiredCaptainCount > 0 ? `${requiredCaptainCount}명` : "산정 불가"}</strong>
        </div>
        <div className="admin-event-detail-card">
          <span>지정 팀장</span>
      ...
      <div className="captain-status-note">
        {getStatusMessage(activeApplications.length, requiredCaptainCount, selectedCount)}
      </div>

      {activeApplications.length === 0 ? (
        <div className="empty-box">
          참가 신청자가 없습니다. 유저 참가 페이지에서 신청을 받은 뒤 팀장을 지정하세요.
        </div>
      ) : null}
      ...
      <div className="destruction-captain-layout">
          ...
          <div className="admin-page__header">
            <div>
              ...
              <p className="admin-page__description">
                기본은 팀장 선호자만 표시합니다. 부족하면 비선호자를 포함해 검색해서 지정하세요.
              </p>
            </div>
          </div>
          ...
          <div className="destruction-candidate-list">
            {candidateApplications.length === 0 ? (
              ...
              candidateApplications.map((application) => {
                const selected = selectedCaptainIds.includes(application.playerId);
                return (
                  <div key={application.id} className="destruction-candidate-row">
                    ...
                    <button
                      type="button"
                      className={selected ? "chip-button danger" : "chip-button"}
        ...
        <section className="destruction-selected-panel">
          <div className="admin-page__header">
            <div>
              <h3 className="admin-event-section-title">지정된 팀장 · 포인트</h3>
              <p className="admin-page__description">
                팀장으로 확정할 사람만 표시합니다. 팀명과 지급 포인트를 여기서 입력합니다.
              </p>
            </div>
          </div>
              ...
              selectedApplications.map((application) => (
                <div key={application.id} className="captain-selected-card">
                  <div className="captain-selected-card__head">
                    <div>
                      ...
                      <p className="admin-page__description" style={{ margin: "4px 0 0" }}>
                        {application.player.nickname}#{application.player.tag} · {application.mainPosition} · {application.isCaptain ? "선호" : "비선호"}
                      </p>
                    </div>
                    <button
                      type="button"
                  ...
                  <div className="captain-selected-card__inputs">
                    <input
                      className="admin-form__input"
                      value={teamNames[application.playerId] ?? toDefaultTeamName(application)}
                      onChange={(event) =>
                        setTeamNames((prev) => ({
                          ...prev,
                    ...
                    <input
                      ...
                      type="number"
                      min="0"
                      value={points[application.playerId] ?? "100"}
                      onChange={(event) =>
                        setPoints((prev) => ({
                          ...prev,
                          [application.playerId]: event.target.value,
          ...
          >
            {isSubmitting ? "저장 중..." : existingTeams.length > 0 ? "팀장/포인트 다시 저장" : "팀 생성 및 경매 시작"}
          </button>
