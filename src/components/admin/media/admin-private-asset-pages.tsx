...
import { AdminContentTabs } from "./admin-media-pages";
...
export async function AdminPrivateAssetListPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <main className={styles.page}><header className={styles.header}><div><strong>보호된 원본 관리</strong><h1>비공개 자산</h1><p>허용된 운영 목적의 메타데이터만 표시하며 저장소 위치와 해시는 노출하지 않습니다.</p></div></header><AdminContentTabs active="private-assets" />
    <form className={styles.filters} action="/admin/private-assets">
      ...
      <input type="hidden" name="pageSize" value={query?.pageSize ?? 24} /><button type="submit" className={styles.submit}>적용</button>
    </form>
...
export async function AdminPrivateAssetDetailPage({ assetId }: { assetId: string }) {
  ...
  return <main className={styles.page}><header className={styles.header}><div><strong>{label(asset.status)}</strong><h1>비공개 자산 상세</h1><p>권한이 확인된 관리자에게 필요한 메타데이터만 보여 줍니다.</p></div><Link href="/admin/private-assets">목록으로</Link></header><AdminContentTabs active="private-assets" />
