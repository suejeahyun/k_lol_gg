...
export async function AdminMediaListPage({ kind, searchParams }: { kind: "highlight" | "gallery"; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <main className={styles.page}><header className={styles.header}><div><strong>콘텐츠·자료</strong><h1>{label} 관리</h1><p>draft, 게시, 보관 상태와 공개 자산 연결을 관리합니다.</p></div><Link href={`${path}/new`}><Plus aria-hidden="true" size={17} /> 새 {label}</Link></header><AdminContentTabs active={kind} /><form className={styles.filters} action={path}><label>게시 상태<select name="status" defaultValue={query?.status ?? ""}><option value="">전체</option><option value="DRAFT">DRAFT</option><option value="PUBLISHED">PUBLISHED</option><option value="ARCHIVED">ARCHIVED</option></select></label><input type="hidden" name="pageSize" value={query?.pageSize ?? 20} /><button type="submit" className={styles.submit}>적용</button></form>
    {result.state !== "ready" ? <section className={styles.state} role={result.state === "error" ? "alert" : "status"}><Sparkles aria-hidden="true" /><h2>{label} 데이터를 불러오지 못했습니다.</h2><p>데이터베이스 연결과 검색 조건을 확인해 주세요.</p></section> : result.data.items.length === 0 ? <section className={styles.state} role="status"><Images aria-hidden="true" /><h2>등록된 {label}가 없습니다.</h2><p>새 초안을 만들면 이곳에서 관리할 수 있습니다.</p></section> : <div className={styles.list}>{result.data.items.map((item) => <Link className={styles.item} href={`${path}/${item.id}/edit`} key={item.id}><div><h2>{item.title}</h2><p>{item.description}</p></div><span className={styles.badge} data-state={item.status}>{item.status}</span><span>rev. {item.revision}</span></Link>)}</div>}
  </main>;
}
