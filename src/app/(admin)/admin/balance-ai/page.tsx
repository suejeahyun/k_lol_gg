  ...
  return (
    <main className={styles.page}>
      ...
      {result.state === "ready" ? <>
        ...
        {result.data.summary.formulaTransition === "ADMIN_RECALCULATION_REQUIRED" ? <section className={styles.notice} role="alert" data-mmr-formula-transition="ADMIN_RECALCULATION_REQUIRED"><h2>MMR 공식 전환 재계산이 필요합니다</h2><p>현재 게시 generation {result.data.summary.generation}은 {result.data.summary.formulaVersion ?? "기존"} 공식으로 계산됐습니다. 자동 전환하지 않으며, SUPER_ADMIN이 기존 공식에서 V2_DETERMINISTIC_1로 전환하는 재계산을 확인해야 새 generation이 게시됩니다.</p></section> : null}
