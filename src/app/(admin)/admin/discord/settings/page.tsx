...
<section className="admin-card discord-ops-panel">
          ...
          <section className="admin-card discord-ops-panel">
            ...
            <form className="discord-settings-form discord-settings-form--readable" onSubmit={handleSave}>
              ...
              <label><span>구인 지각 자동 경고</span><select className="admin-input" value={settings.recruitLateWarningEnabled ? "true" : "false"} onChange={(e) => setSettings({ ...settings, recruitLateWarningEnabled: e.target.value === "true" })}><option value="true">ON</option><option value="false">OFF</option></select></label>
              <label><span>구인 지각 유예 분</span><input className="admin-input" type="number" min="0" max="120" value={settings.recruitLateWarningGraceMinutes} onChange={(e) => setSettings({ ...settings, recruitLateWarningGraceMinutes: Number(e.target.value) })} /></label>
              <label><span>구인 지각 개인 DM</span><select className="admin-input" value={settings.recruitLateWarningDmEnabled ? "true" : "false"} onChange={(e) => setSettings({ ...settings, recruitLateWarningDmEnabled: e.target.value === "true" })}><option value="true">ON</option><option value="false">OFF</option></select></label>
