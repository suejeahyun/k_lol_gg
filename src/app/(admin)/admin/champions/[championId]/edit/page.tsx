...
export default async function EditChampionPage({ params }: { params: Promise<{ championId: string }> }) {
  ...
  if (result.state !== "ready") return <main className={styles.page}><section className={styles.state} role={result.state === "error" ? "alert" : "status"}><Sparkles aria-hidden="true" /><h1>챔피언을 불러오지 못했습니다.</h1><p>데이터베이스 연결을 확인한 뒤 다시 시도해 주세요.</p></section></main>;
  return <main className={styles.page}><header className={styles.header}><div><strong>{result.data!.status} · rev. {result.data!.revision}</strong><h1>{result.data!.displayName}</h1><p>키는 경기 원장 참조를 위해 바꾸지 않으며 삭제 대신 비활성화합니다.</p></div></header><AdminContentTabs active="champion" /><ChampionForm key={`${result.data!.key}-${result.data!.revision}`} initial={result.data!} /></main>;
}
