...
import styles from "../../kakao/AdminKakaoReadable.module.css";
...
export default async function AdminRecruitLogsPage({ searchParams }: PageProps) {
  ...
  recentFrom.setDate(recentFrom.getDate() - 1);
    ...
    <main className="admin-page">
      <div className={styles.shell}>
        <div className={styles.header}>
          <div>
            <p className={styles.kicker}>RECRUIT LOG</p>
            <h1 className={styles.title}>구인구직 기록</h1>
          </div>
        </div>
        ...
        <section className={styles.statsGrid}>
          <div className={styles.statCard}>
            ...
            <strong className={styles.statValue}>{totalCount.toLocaleString("ko-KR")}</strong>
            <div className={styles.statHint}>현재 필터</div>
          </div>
          <div className={styles.statCard}>
            <span className={styles.statLabel}>최근 24시간</span>
            <strong className={styles.statValue}>{recentCount.toLocaleString("ko-KR")}</strong>
            <div className={styles.statHint}>전체 기록</div>
          </div>
          <div className={styles.statCard}>
            <span className={styles.statLabel}>자동 처리</span>
            <strong className={styles.statValue}>{autoCount.toLocaleString("ko-KR")}</strong>
            <div className={styles.statHint}>AUTO</div>
          </div>
          <div className={styles.statCard}>
            <span className={styles.statLabel}>마감 기록</span>
            <strong className={styles.statValue}>{finishedCount.toLocaleString("ko-KR")}</strong>
            <div className={styles.statHint}>FINISH</div>
          </div>
        </section>

        <form className={styles.filterCard} action="/admin/recruits/logs">
          <div className={styles.filterGrid}>
            ...
            <a className={styles.secondaryButton} href="/admin/recruits/logs">초기화</a>
          </div>
        </form>
        ...
        <section className={styles.card}>
          <div className={styles.cardHead}>
            <div>
              <h2 className={styles.cardTitle}>로그 목록</h2>
              <p className={styles.cardMeta}>페이지당 {PAGE_SIZE}개</p>
            </div>
          </div>
            ...
            <table className={styles.table}>
              <thead>
                <tr>
                  ...
                  <th className={styles.colSmall}>회차</th>
                  <th className={styles.colSmall}>번호</th>
                  <th className={styles.colAction}>액션</th>
                  <th className={styles.colTitle}>제목</th>
                  <th className={styles.colPeople}>인원</th>
                  <th className={styles.colSource}>방/처리자</th>
                  <th className={styles.colSummary}>요약</th>
                </tr>
              </thead>
              <tbody>
                {logs.length === 0 ? (
                  <tr>
                    <td className={styles.empty} colSpan={8}>구인구직 기록이 없습니다.</td>
                  </tr>
                ) : (
                  logs.map((log) => {
                    ...
                    return (
                      <tr key={log.id}>
                        <td data-label="시간">
                          <span className={styles.dateStack}>{dateParts.dateText}<small>{dateParts.timeText}</small></span>
                        </td>
                        ...
                        <td data-label="방/처리자">
                          <div className={styles.primaryText}>{log.roomName || "-"}</div>
                          <div className={styles.muted}>{log.sender || "-"}</div>
                        </td>
          ...
          <div className={styles.paginationWrap}>
            <Pagination
              ...
              basePath="/admin/recruits/logs"
              query={{ q: q || undefined, date: date || undefined, action: action || undefined }}
            />
          </div>
        </section>
      </div>
