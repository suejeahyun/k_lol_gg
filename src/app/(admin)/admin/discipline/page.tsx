...
export default async function AdminDisciplinePage({ searchParams }: { searchParams: Promise<{ tab?: string | string[] }> }) {
  ...
  return <main className={styles.page}><header className={styles.header}><div><span className={styles.eyebrow}>관리자 · 징계</span><h1>징계 관리</h1><p>기록, 해소 과제와 검토 대기를 한곳에서 관리합니다.</p></div><Link className={styles.button} href="/admin/discipline/new">새 기록</Link></header><nav className={styles.tabs} aria-label="징계 관리 분류"><Link href="/admin/discipline" aria-current={tab === "records" ? "page" : undefined}>전체 기록</Link><Link href="/admin/discipline?tab=tasks" aria-current={tab === "tasks" ? "page" : undefined}>과제</Link><Link href="/admin/discipline?tab=reviews" aria-current={tab === "reviews" ? "page" : undefined}>검토 대기</Link></nav>
    {result.state === "ready" && result.data.items.length ? <div className={styles.tableWrap} role="region" aria-label="징계 기록 목록" tabIndex={0}><table className={styles.table}><thead><tr><th scope="col">대상</th><th scope="col">유형</th><th scope="col">상태</th><th scope="col">과제</th><th scope="col">등록일</th><th scope="col">보기</th></tr></thead><tbody>{result.data.items.map((record) => <tr key={record.id}><td>{record.targetName}</td><td>{typeLabel[record.type]}</td><td><span className={styles.badge} data-status={record.active ? "ACTIVE" : "INACTIVE"}>{record.active ? "활성" : "종료"}</span></td><td>{record.task ? taskStatusLabel[record.task.status] : "없음"}</td><td>{new Intl.DateTimeFormat("ko-KR", { dateStyle: "short" }).format(new Date(record.createdAt))}</td><td><Link href={`/admin/discipline/${record.id}`}>상세</Link></td></tr>)}</tbody></table></div> : result.state === "ready" ? <section className={styles.state}><h2>이 분류의 기록이 없습니다.</h2><p>필터를 바꾸거나 새 기록을 등록해 주세요.</p></section> : result.state === "unavailable" ? <section className={styles.state} role="status">징계 기록을 확인할 수 없습니다.</section> : <section className={styles.state} role="alert">징계 기록을 불러오지 못했습니다.</section>}
  </main>;
}
