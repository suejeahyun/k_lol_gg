...
const categoryLabel = { GENERAL: "일반", INHOUSE: "내전" } as const;
...
const evidenceStatusLabel = { STAGED: "검사 중", READY: "저장 완료", DELETE_PENDING: "삭제 대기" } as const;
...
export default async function AdminDisciplineDetailPage({ params }: { params: Promise<{ recordId: string }> }) {
  ...
  return <main className={styles.page}><header className={styles.header}><div><span className={styles.eyebrow}>관리자 · 상세</span><h1>{record.targetName}</h1><p>{typeLabel[record.type]} · 변경 버전 {record.revision}</p></div><Link className={styles.link} href="/admin/discipline">목록으로</Link></header><div className={styles.detailGrid}><section className={styles.panel}><h2>기록</h2><dl className={styles.facts}><div><dt>분류</dt><dd>{categoryLabel[record.category]}</dd></div><div><dt>출처</dt><dd>{record.source}</dd></div><div><dt>활성</dt><dd>{record.active ? "활성" : "종료"}</dd></div><div><dt>등록</dt><dd>{new Intl.DateTimeFormat("ko-KR", { dateStyle: "medium", timeStyle: "short", timeZone: "Asia/Seoul" }).format(new Date(record.createdAt))}</dd></div></dl><h3>사유</h3><p>{record.reason}</p></section><section className={styles.panel}><h2>해소 과제</h2>{record.task ? <><p><span className={styles.badge}>{taskStatusLabel[record.task.status]}</span> · {record.task.submittedEvidenceCount}/{record.task.requiredGameCount}장</p>{record.task.reviewNote ? <p className={styles.notice}>{record.task.reviewNote}</p> : null}<div className={styles.evidence}>{record.task.evidence.map((item, index) => <a key={item.assetId} href={`/api/admin/discipline/assets/${item.assetId}`} target="_blank" rel="noreferrer">증거 {index + 1}<br />{evidenceStatusLabel[item.status]}</a>)}</div></> : <p className={styles.muted}>연결된 과제가 없습니다.</p>}</section></div><AdminDisciplineActions record={record} canManage={session.role === "SUPER_ADMIN"} /></main>;
}
