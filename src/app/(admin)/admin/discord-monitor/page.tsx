  ...
  return (
    <main className="admin-page discord-monitor-page">
      <div className="admin-page__header">
        <div>
          <h1 className="admin-page__title">디스코드 운영 모니터 v2</h1>
          <p className="admin-page__description">
            동적 음성방, 구인 자동 ㅉ, 입퇴장 로그, 운영진 전용 통계를 한 화면에서 확인합니다.
          </p>
        </div>
      </div>
      ...
      <form className="discord-filter-card" method="get">
        ...
        <Link href="/admin/discord-monitor">초기화</Link>
      </form>
      ...
      <style>{`
        ...
        @media (max-width: 1100px) { .discord-stat-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); } .discord-chart-grid { grid-template-columns: 1fr; } .discord-filter-card { grid-template-columns: 1fr; } }
      `}</style>
    </main>
  );
}
