...
export default async function AdminBalanceRecommendationsIndexPage({ searchParams }: Props) {
    ...
    <main className="page-container ai-page">
      <section className="ai-hero">
        <div className="ai-hero__content">
          <p className="eyebrow">ADMIN · PICK · BAN RECOMMENDATIONS</p>
          <h1 className="page-title">밴픽 추천</h1>
          <p className="page-description">
            저장된 팀 밸런스 결과를 선택해 밴 우선순위, 플레이어별 픽 추천, 포지션 페어 호흡, 조합 방향을 확인합니다.
          </p>
          <div className="ai-hero__actions">
            <Link className="button-secondary" href="/admin/balance/drafts">저장 밸런스</Link>
            <Link className="button-primary" href="/admin/balance">새 팀 밸런스 계산</Link>
          </div>
        </div>
      </section>
      ...
      <section className="ai-kpi-grid">
        ...
        <article className="ai-kpi"><span>권한</span><strong>관리자</strong><small>관리자 전용 진입</small></article>
      </section>

      <section className="ai-panel ai-panel--strong">
        <div className="ai-panel__head">
          <div>
            <h2 className="ai-panel__title">좌측 사이드바 진입 페이지</h2>
            <p className="ai-panel__desc">관리자 사이드바의 팀 밸런스 영역에서 직접 들어오는 밴픽 추천 목록입니다.</p>
          </div>
        </div>
            ...
            <article className="ai-panel ai-draft-card" key={draft.id}>
              <div className="ai-panel__head">
                <div>
                  ...
                  <p className="ai-panel__desc">{draft.season?.name ?? "시즌 없음"} · {formatDate(draft.createdAt)}</p>
                </div>
      ...
      <Pagination currentPage={page} totalPages={totalPages} basePath="/admin/balance/recommendations" />
    </main>
  );
}
