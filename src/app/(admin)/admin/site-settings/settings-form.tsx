...
export default function SettingsForm({ initial }: { initial: SiteSettings }) {
  ...
  return (
    <form className={styles.form} onSubmit={submit}>
      ...
      <div className={styles.actions}><button className={styles.button} disabled={state === "saving"}>{state === "saving" ? "저장 중…" : "설정 저장"}</button><span className={styles.muted}>revision {settings.revision}</span></div>
    </form>
  );
}
