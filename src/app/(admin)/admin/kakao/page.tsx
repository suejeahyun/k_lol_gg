...
export default async function AdminKakaoPage({
  ...
  return <main className={styles.page}>
    ...
    {result.state === "ready" ? <>
      ...
      {tab === "settings" ? settingsResult.state === "ready" ? <section className={styles.panel}><div className={styles.panelHead}><h2>비밀값 제외 운영 설정</h2><span>{session.role === "SUPER_ADMIN" ? "SUPER 수정 가능" : "읽기 전용"}</span></div><div className={styles.settingsBody}><p>서명 키와 비상 bootstrap 방·발신자 원문은 서버 환경변수에만 두며 화면과 API에 노출하지 않습니다. 정상 방 권한은 canonical DB registry에서 관리합니다.</p>{session.role === "SUPER_ADMIN" ? <KakaoSettingsForm initial={settingsResult.data}/> : <p>설정 변경은 SUPER 관리자와 2단계 인증이 필요합니다.</p>}</div></section> : <section className={styles.state} role={settingsResult.state === "error" ? "alert" : "status"}><DatabaseZap/><h2>운영 설정을 불러올 수 없습니다.</h2><p>0022 migration과 데이터베이스 상태를 확인해 주세요.</p></section> : null}
      {tab === "health" ? <><section className={styles.health}><article><ShieldCheck/><div><h2>서명·비상 bootstrap</h2><p>현재 키 {runtimeConfiguration.currentSigningKeyConfigured ? "설정" : "미설정"} · bootstrap 방 {runtimeConfiguration.allowedRoomsConfigured ? "설정" : "미설정"} · 발신자 {runtimeConfiguration.allowedSendersConfigured ? "설정" : "미설정"}</p></div></article><article><Clock3/><div><h2>nonce·영수증</h2><p>활성 nonce {result.data.activeNonceCount} · 미완료 영수증 {result.data.incompleteReceiptCount}</p></div></article><article><DatabaseZap/><div><h2>세션·outbox</h2><p>활성 이미지 세션 {result.data.activeImageSessionCount} · 대기 outbox {result.data.pendingOutboxCount}</p></div></article></section>{session.role === "SUPER_ADMIN" && settingsResult.state === "ready" ? <section className={styles.panel}><div className={styles.panelHead}><h2>안전 복구 작업</h2><span>SUPER · 2단계 인증</span></div><div className={styles.settingsBody}><p>만료 시간이 지난 활성 이미지 수신 세션만 EXPIRED 상태로 전환합니다. 자산이나 신청 데이터는 삭제하지 않습니다.</p><KakaoHealthRepair revision={settingsResult.data.revision}/></div></section> : null}</> : null}
