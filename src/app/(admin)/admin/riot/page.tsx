...
export default async function AdminRiotPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.card}><h2>연동 작업</h2><p>단일 연결은 ADMIN, 선택 일괄·전체 동기화는 SUPER_ADMIN 세션과 TOTP를 transaction 안에서 다시 확인합니다.</p><RiotAdminGlobalActions superAdmin={session.role === "SUPER_ADMIN"} /></section>
