...
function pageHref(query: NonNullable<ReturnType<typeof parseAdminRiotQuery>>, page: number) {
  const params = new URLSearchParams({ tab: query.tab, page: String(page), pageSize: String(query.pageSize) });
  params.set(query.tab === "logs" ? "source" : "status", query.tab === "logs" ? query.source : query.status);
  return `/admin/riot?${params.toString()}`;
}
...
export default async function AdminRiotPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const parsedQuery = parseAdminRiotQuery(url.href);
  ...
  const session = await requirePageRole(query.action === "bulk-link" ? "SUPER_ADMIN" : "ADMIN", "/admin/riot");
  if (!parsedQuery) return <main className={styles.page} data-riot-state="invalid-query"><section className={styles.state} data-tone="error" role="alert"><h1>Riot 운영 주소가 올바르지 않습니다.</h1><p>중복되거나 허용되지 않은 필터를 제거한 뒤 다시 시도해 주세요.</p><Link href="/admin/riot?tab=accounts">계정 화면으로 돌아가기</Link></section></main>;
  ...
  const pageCount = ready ? Math.min(ready.totalPages, Math.floor(5_000 / query.pageSize)) : 0;
  const rangeStart = count ? (query.page - 1) * query.pageSize + 1 : 0;
  const rangeEnd = count ? rangeStart + count - 1 : 0;
  const countUnit = query.tab === "accounts" ? "명" : "건";
  ...
  return <main className={styles.page} data-riot-tab={query.tab} data-riot-action={query.action}>
    ...
    {ready && query.action === "NONE" ? <div className={styles.listSummary} aria-label="조회 범위">
      <p>현재 필터 전체 <strong>{ready.total.toLocaleString("ko-KR")}{countUnit}</strong> · {count ? `${rangeStart.toLocaleString("ko-KR")}–${rangeEnd.toLocaleString("ko-KR")}${countUnit} 표시` : `현재 페이지 0${countUnit}`}</p>
      {ready.totalPages > pageCount ? <p>최대 5,000건까지 페이지로 조회할 수 있습니다. 상태나 로그 구분을 선택해 범위를 좁혀 주세요.</p> : null}
    </div> : null}
    ...
    {query.tab === "accounts" ? <>
      <section className={styles.card} data-riot-state="accounts"><h2>Riot 계정 연결 현황</h2><p>승인된 플레이어의 연결 상태를 찾고, 단일 연결 또는 선택 일괄 동기화를 진행합니다.</p>
        {query.action === "bulk-link" ? <form className={styles.filterForm} method="get"><input type="hidden" name="tab" value="accounts" /><input type="hidden" name="action" value="bulk-link" /><label htmlFor="riot-bulk-query">활성 미연동 플레이어 검색</label><input id="riot-bulk-query" name="q" defaultValue={query.q} maxLength={100} placeholder="이름 또는 닉네임#태그" /><label htmlFor="riot-bulk-size">배치 크기</label><input id="riot-bulk-size" name="batchSize" type="number" min={1} max={30} defaultValue={query.batchSize} /><button type="submit">미리보기 갱신</button></form>
          : <form className={styles.filterForm} method="get"><input type="hidden" name="tab" value="accounts" /><input type="hidden" name="pageSize" value={query.pageSize} /><label htmlFor="riot-account-status">계정 상태</label><select id="riot-account-status" name="status" defaultValue={query.status}>{accountStatuses.map((status) => <option key={status} value={status}>{accountStatusLabel[status]}</option>)}</select><button type="submit">조회</button></form>}
        {query.action === "bulk-link"
          ...
          : <><p className={styles.selectionSummary}>전체 동기화는 현재 필터·페이지와 관계없이 연결됨 상태의 모든 계정을 대상으로 합니다. 연결 안 됨·연결 해제·연결 취소 계정은 연결 후 동기화할 수 있습니다.</p><RiotAdminGlobalActions superAdmin={session.role === "SUPER_ADMIN"} items={ready?.items ?? []} />{session.role === "SUPER_ADMIN" ? <p><Link href="/admin/riot?tab=accounts&action=bulk-link">활성 미연동 플레이어 일괄 연결</Link></p> : null}</>}
      </section>
    ...
    {query.tab === "sync" ? <section data-riot-state="sync"><div className={styles.card}><h2>Riot 동기화 작업 이력</h2><p>대기·실행·재시도·완료 상태와 시도 횟수를 실제 작업 단위로 확인합니다.</p><form className={styles.filterForm} method="get"><input type="hidden" name="tab" value="sync" /><input type="hidden" name="pageSize" value={query.pageSize} /><label htmlFor="riot-sync-status">작업 상태</label><select id="riot-sync-status" name="status" defaultValue={query.status}>{syncStatuses.map((status) => <option key={status} value={status}>{syncStatusLabel[status]}</option>)}</select><button type="submit">조회</button></form></div>
    ...
    {query.tab === "logs" ? <section data-riot-state="logs"><div className={styles.card}><h2>Riot API·동기화·감사 로그</h2><p>보호된 식별자나 인증 정보를 노출하지 않고 API 결과, 이벤트 발행, 관리자 감사 기록을 확인합니다.</p><form className={styles.filterForm} method="get"><input type="hidden" name="tab" value="logs" /><input type="hidden" name="pageSize" value={query.pageSize} /><label htmlFor="riot-log-source">로그 구분</label><select id="riot-log-source" name="source" defaultValue={query.source}>{logSources.map((source) => <option key={source} value={source}>{logSourceLabel[source]}</option>)}</select><button type="submit">조회</button></form></div>
    ...
    {ready && query.action === "NONE" && (pageCount > 1 || query.page > 1) ? <nav className={styles.pagination} aria-label="Riot 목록 페이지">
      {query.page > 1 ? <Link href={pageHref(query, Math.max(1, Math.min(query.page - 1, pageCount)))}>이전</Link> : <span aria-disabled="true">이전</span>}
      <span aria-current="page">{query.page <= pageCount ? `${query.page} / ${pageCount} 페이지` : "현재 페이지에 결과 없음"}</span>
      {query.page < pageCount ? <Link href={pageHref(query, query.page + 1)}>다음</Link> : <span aria-disabled="true">다음</span>}
    </nav> : null}
  </main>;
}
