      ...
      <div className={styles.gameStack}>
        {form.games.map((game, gameIndex) => {
          ...
          return <article className={styles.gameEditor} key={gameIndex}>
            <header className={styles.gameEditorHeader}><div><span>GAME {gameIndex + 1}</span><h3>{gameIndex + 1}게임</h3></div><div className={styles.compactControls}><label>승리 팀<select value={game.winnerTeam} onChange={(event) => updateGame(gameIndex, (current) => ({ ...current, winnerTeam: event.target.value as EditableGame["winnerTeam"] }))}><option value="BLUE">블루</option><option value="RED">레드</option></select></label><label>진행 초<input type="number" min="60" max="7200" value={game.durationSeconds} onChange={(event) => updateGame(gameIndex, (current) => ({ ...current, durationSeconds: Number(event.target.value) }))} /></label>{form.games.length > 1 ? <button type="button" className={styles.iconButton} aria-label={`${gameIndex + 1}게임 삭제`} onClick={() => setForm((current) => ({ ...current, games: current.games.filter((_, index) => index !== gameIndex).map((item, index) => ({ ...item, gameNumber: index + 1 })) }))}><Trash2 size={17} aria-hidden="true" /></button> : null}</div></header>
