...
export function DestructionCreateForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      pending.current ??= { tournamentId, key: `destruction-create-${crypto.randomUUID()}`, body: JSON.stringify({ tournamentId, title: data.get("title"), configuration: { gameMode: data.get("gameMode"), preliminaryFormat: data.get("preliminaryFormat"), preliminaryRoundCount: Number(data.get("preliminaryRoundCount")), teamCount, laneLimits } }) };
  ...
  return <section className={styles.workspace}><form className={`${styles.panel} ${styles.form}`} onSubmit={submit}>
    ...
    <fieldset><legend>포지션별 모집 상한</legend>{positions.map((position) => <label key={position}>{competitionPositionLabel(position)}<input name={position} type="number" min={Math.max(4, teamCount)} max={99} defaultValue={8} required /></label>)}</fieldset>
    <p>모집 상한은 심사 대기·예비·참가 확정 인원을 합산합니다. 각 포지션에서 팀 수만큼 참가자를 확정하면 모집을 마감할 수 있습니다.</p>
