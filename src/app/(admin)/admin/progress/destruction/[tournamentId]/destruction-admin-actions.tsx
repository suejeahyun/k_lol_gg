...
import { aramAuctionRating, DESTRUCTION_GAME_MODES } from "@/modules/competitions/destruction/aram-rating";
...
export function DestructionAdminActions({ destruction, playerOptions, playerLabels, galleryOptions, view = "default", selectedStage, superAdmin = false }: Readonly<{
  ...
  selectedStage?: (typeof DESTRUCTION_STEPS)[number];
  superAdmin?: boolean;
}>) {
...
function StageRecords(context: Context & { stage: (typeof DESTRUCTION_STEPS)[number] }) {
  const { destruction, stage, playerLabel, teamLabel } = context;
  if (stage === "PLANNED") return <section className={styles.panel}><h3>대회 설정·일정</h3><p>{DESTRUCTION_GAME_MODES[destruction.configuration.gameMode ?? "CLASSIC"]} · {destruction.configuration.teamCount}팀 · 팀당 5명 · {(destruction.configuration.gameMode ?? "CLASSIC") === "CLASSIC" ? "포지션별 1명" : "포지션 구분 없음"} · 본선 BO3</p>{Object.entries(DESTRUCTION_SCHEDULE_LABELS).map(([key, label]) => { const instant = destruction.schedule?.[key as keyof typeof DESTRUCTION_SCHEDULE_LABELS]; return <p key={key}>{label}: {instant ? new Intl.DateTimeFormat("ko-KR", { timeZone: "Asia/Seoul", dateStyle: "medium", timeStyle: "short" }).format(new Date(instant)) : "미정"}</p>; })}</section>;
  if (stage === "RECRUITING") return <Recruitment {...context} readOnly />;
  if (stage === "TEAM_BUILDING") return <section className={styles.panel}><h3>주장·포인트 확정 기록</h3>{destruction.teams.length ? destruction.teams.map((team) => <p key={team.id}>{team.name} · 주장 {playerLabel(destruction.participants.find((p) => p.teamId === team.id && p.isCaptain)?.playerId ?? "")} · 시작 {team.initialAuctionPoints}P</p>) : <p>주장과 팀이 아직 확정되지 않았습니다.</p>}{destruction.participants.filter((p) => p.aramRecord).map((p) => { const rating = aramAuctionRating(p.aramRecord!); return <p key={p.id}>{playerLabel(p.playerId)} · {rating.tier}등급 · {rating.games}판 {rating.wins}승 {rating.losses}패 · 최소 {rating.minimumBid}P · {p.aramRecord!.source === "RIOT" ? "Riot 조회" : "운영자 확인"}</p>; })}</section>;
  if (stage === "AUCTION") return <section className={styles.panel}><h3>선수 경매 기록</h3><AuctionReveal card={destructionAuctionCard(destruction, playerLabel)} />{destruction.participants.length ? <div className={styles.tableWrap}><table><caption>참가 선수의 경매·배정 현황</caption><thead><tr><th>선수</th>{(destruction.configuration.gameMode ?? "CLASSIC") === "CLASSIC" ? <th>포지션</th> : null}<th>팀</th><th>낙찰 포인트</th></tr></thead><tbody>{destruction.participants.map((p) => <tr key={p.id}><th scope="row">{playerLabel(p.playerId)}</th>{(destruction.configuration.gameMode ?? "CLASSIC") === "CLASSIC" ? <td>{competitionPositionLabel(p.position)}</td> : null}<td>{p.teamId ? teamLabel(p.teamId) : "미배정"}</td><td>{p.isCaptain ? "주장" : p.purchasePoints === null ? "낙찰 전" : p.purchasePoints + "P"}</td></tr>)}</tbody></table></div> : <p>아직 경매 기록이 없습니다.</p>}</section>;
  const preliminary = destruction.preliminaryFixtures.map((f) => ({ ...f, label: "예선" }));
  const tournament = (destruction.tournamentBracket?.fixtures ?? []).map((f) => ({ ...f, label: f.stage === "FINAL" ? "결승" : "본선", teamAScore: f.result?.teamAScore ?? null, teamBScore: f.result?.teamBScore ?? null }));
  const fixtures = stage === "PRELIMINARY" ? preliminary : stage === "TOURNAMENT" ? tournament : [...preliminary, ...tournament];
  return <section className={styles.panel}><h3>{stage === "COMPLETED" ? "최종 결과·MVP" : stage === "PRELIMINARY" ? "예선 대진·결과" : "본선 대진·결과"}</h3>{stage === "COMPLETED" ? <p>{destruction.lifecycle.status === "COMPLETED" ? "대회가 종료되었습니다." : "아직 종료되지 않은 대회입니다."} 우승: {destruction.tournamentBracket?.championTeamId ? teamLabel(destruction.tournamentBracket.championTeamId) : "미정"}</p> : null}{fixtures.length ? fixtures.map((f) => { const mvp = destruction.mvpBallots.find((b) => b.fixtureId === f.id)?.finalizedPlayerId; return <article className={styles.callout} key={f.id}><h4>{f.label} · BO{f.bestOf} · {f.teamAId ? teamLabel(f.teamAId) : "진출 팀 미정"} vs {f.teamBId ? teamLabel(f.teamBId) : "진출 팀 미정"}</h4><p>{f.teamAScore === null ? "결과 대기" : f.teamAScore + " : " + f.teamBScore} · MVP {mvp ? playerLabel(mvp) : "미정"}</p></article>; }) : <p>아직 공개된 대진이 없습니다.</p>}</section>;
}
...
function ReplacementForm({ destruction, busy, command, playerLabel, teamLabel, superAdmin, playerOptions }: Context & { playerOptions: readonly CompetitionPlayerOption[] }) {
  ...
  return <section className={styles.panel}><h2>선수 교체 · SUPER</h2><p>기존 경기 로스터는 보존됩니다. 다음 경기부터 새 선수가 반영됩니다. 협곡은 기존 포지션을 유지합니다.</p><form className={styles.form} onSubmit={(event) => { const data = formData(event); if (participant) void command("REPLACE_PARTICIPANT", { replacementId: crypto.randomUUID(), participantId, incomingPlayerId, incomingPosition: (destruction.configuration.gameMode ?? "CLASSIC") === "CLASSIC" ? participant.position : null, reason: data.get("reason") }); }}><label>교체 대상<select value={participantId} onChange={(event) => setParticipantId(event.target.value)}>{destruction.participants.filter((entry) => entry.teamId).map((entry) => <option key={entry.id} value={entry.id}>{teamLabel(entry.teamId!)} · {playerLabel(entry.playerId)}{(destruction.configuration.gameMode ?? "CLASSIC") === "CLASSIC" ? ` · ${competitionPositionLabel(entry.position)}` : ""}</option>)}</select></label><label>투입 선수 검색<BoundedPicker ariaLabel="교체 투입 선수" value={incomingPlayerId} options={playerOptions} disabledValues={new Set(destruction.participants.map((entry) => entry.playerId))} placeholder="닉네임 또는 태그 검색" remoteEndpoint="/api/admin/matches/editor-options/players" onChange={setIncomingPlayerId} /></label><label>교체 사유<input name="reason" minLength={2} maxLength={500} required /></label><button disabled={busy || !superAdmin || !incomingPlayerId || !participant}>선수 교체</button>{!superAdmin ? <p>SUPER_ADMIN 권한이 필요합니다.</p> : null}</form></section>;
}
