...
export function DestructionAdminActions({ destruction, playerOptions, playerLabels, galleryOptions, view = "default" }: Readonly<{
  ...
  useEffect(() => {
    if (view !== "auction-live" || busy) return;
  ...
  if (view === "auction-live") return <section className={styles.actions} aria-labelledby="auction-live-actions-title"><h2 id="auction-live-actions-title">실시간 경매 작업</h2><p aria-live="polite">8초마다 최신 경매 상태를 다시 확인합니다. 모든 작업은 기존 ADMIN·TOTP·revision·멱등성 계약으로 처리됩니다.</p><div className={styles.actionsGrid}>
    <article className={styles.form}><strong>현재 추첨 선수</strong><p>{status === "AUCTION" ? drawn ? participantLabel(drawn.id) : "추첨 대기 중" : `현재 단계: ${status}`}</p><button type="button" onClick={() => router.refresh()} disabled={busy}>지금 새로고침</button>{status === "AUCTION" ? <><button className={styles.primary} disabled={busy || Boolean(drawn)} onClick={() => void command("DRAW_AUCTION", {})}>다음 선수 추첨</button>{drawn ? <button disabled={busy} onClick={() => void command("HOLD_AUCTION", { participantId: drawn.id })}>이번 선수 보류</button> : null}</> : <p>경매 단계에서만 추첨·낙찰할 수 있습니다.</p>}</article>
    {status === "AUCTION" && drawn ? <form className={styles.form} onSubmit={(event) => { const data = formPayload(event); void command("SELL_AUCTION", { participantId: drawn.id, teamId: data.get("teamId"), purchasePoints: Number(data.get("purchasePoints")) }); }}><strong>{participantLabel(drawn.id)} 낙찰</strong><label>낙찰 팀<select name="teamId">{destruction.teams.map((team) => <option key={team.id} value={team.id}>{team.name} · 잔여 {team.remainingAuctionPoints}P</option>)}</select></label><label>낙찰 포인트<input name="purchasePoints" type="number" min={1} max={Math.max(...destruction.teams.map((team) => team.remainingAuctionPoints), 1)} required /></label><button disabled={busy}>낙찰 확정</button></form> : null}
    {status === "AUCTION" ? <button className={styles.primary} disabled={busy} onClick={() => void command("PUBLISH_PRELIMINARY", {})}>경매 완료·예선 대진 공개</button> : null}
