  ...
  return <AccountShell activeTab="riot" title="내 Riot 계정" description="승인된 내 플레이어에 Riot ID를 연결하고 안전하게 전적 동기화를 요청합니다." status={{ label: link ? publicRiotLinkStatusLabel(link.status) : "승인된 계정", state: link?.status ?? "APPROVED" }} action={<Link className={styles.primaryLink} href="/help/riot">연동 도움말</Link>}>
    {result.state === "unavailable" ? <section className={styles.panel} role="status"><h2>Riot 연동을 준비하고 있어요.</h2><p>이용 가능해지면 이 화면에서 계정을 연결할 수 있습니다.</p></section>
      ...
      : <div className={styles.accountGrid}>
        <section className={styles.panel}><h2>{result.data.link ? "연결된 Riot 계정" : "Riot 계정 연결"}</h2>{result.data.link ? <dl className={styles.facts}><div><dt>Riot ID</dt><dd>{result.data.link.riotId}</dd></div><div><dt>연결 방식</dt><dd>{publicRiotLinkMethodLabel(result.data.link.method)}</dd></div><div><dt>상태</dt><dd>{publicRiotLinkStatusLabel(result.data.link.status)}</dd></div></dl> : <p>Riot ID를 직접 입력하거나 Riot 계정으로 확인할 수 있습니다.</p>}<RiotOwnerActions linkRevision={result.data.link?.revision ?? 0} connected={result.data.link?.status === "CONNECTED"} /></section>
