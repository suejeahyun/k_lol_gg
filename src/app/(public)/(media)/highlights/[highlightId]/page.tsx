...
export default async function HighlightDetail({ params }: { params: Promise<{ highlightId: string }> }) {
  ...
  return <article className={`page-wrap ${styles.detail}`}><header className={styles.detailHeader}><span className={styles.tag}>HIGHLIGHT</span><h1>{item.title}</h1><p>{item.description}</p></header><div className={styles.player}><iframe src={`https://www.youtube-nocookie.com/embed/${item.youtubeId}`} title={item.title} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen referrerPolicy="strict-origin-when-cross-origin" /></div><div><Link className={styles.back} href="/highlights"><ArrowLeft size={17} /> 목록으로</Link></div></article>;
}
