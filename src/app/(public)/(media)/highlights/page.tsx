...
export default async function HighlightsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <div className={`page-wrap ${styles.page}`}>
    ...
    {result.state === "unavailable" ? <State icon={<CloudSun />} title="하이라이트를 확인할 수 없어요." body="잠시 후 다시 확인해 주세요." />
      ...
      : <><div className={styles.grid}>{items.map((item) => <Link className={styles.card} data-media-kind="highlight" href={`/highlights/${item.id}`} key={item.id}><div className={styles.visual}><ResilientMediaImage sizes="(max-width: 640px) 100vw, (max-width: 900px) 50vw, 33vw" src={item.thumbnailUrl} alt="" /><span className={styles.playCue}><Play aria-hidden="true" /></span></div><div className={styles.cardBody}><span className={styles.tag}>PLAY</span><h2>{item.title}</h2><p>{item.description}</p></div></Link>)}</div>{result.data.nextCursor ? <nav className={styles.pager}><Link href={`/highlights?cursor=${result.data.nextCursor}&pageSize=${query?.pageSize ?? 12}`}>다음 영상 보기</Link></nav> : null}</>}
  </div>;
}
