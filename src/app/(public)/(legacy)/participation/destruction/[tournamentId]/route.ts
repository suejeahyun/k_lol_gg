...
async function redirect(context: { params: Promise<{ tournamentId: string }> }) { return legacyRedirectResponse(buildLegacyCanonicalIdDestination("/competitions/destruction", (await context.params).tournamentId, "/applications", { action: "apply" })); }
