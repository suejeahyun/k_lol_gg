...
export default async function EventDetailPage({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.facts} aria-label="이벤트전 일정"><article><CalendarDays aria-hidden="true" /><div><span>모집 기간</span><strong>{new Date(event.recruitmentOpensAt).toLocaleString("ko-KR")}<br />~ {new Date(event.recruitmentClosesAt).toLocaleString("ko-KR")}</strong></div></article><article><UsersRound aria-hidden="true" /><div><span>현재 참가자</span><strong>{event.participantCount}/10명</strong></div></article><article><Crown aria-hidden="true" /><div><span>우승 팀</span><strong>{event.winnerTeamId ?? "아직 결정 전"}</strong></div></article></section>
    ...
    <section className={styles.detailGrid}><article><h2>팀 편성</h2>{event.teams.length ? event.teams.map((team) => <div className={styles.team} key={team.id}><strong>{team.name}</strong><ul>{team.members.map((member) => <li key={member.participantId}><span>{member.position ?? "ARAM"}</span>{member.playerId}</li>)}</ul></div>) : <p>아직 팀을 편성하지 않았어요.</p>}</article><article><h2>대진과 결과</h2>{event.fixtures.length ? event.fixtures.map((fixture) => <div className={styles.fixture} key={fixture.id}><span>ROUND {fixture.roundNumber} · BO{fixture.bestOf}</span><strong>{fixture.teamAId ?? "미정"} {fixture.teamAScore ?? "-"} : {fixture.teamBScore ?? "-"} {fixture.teamBId ?? "미정"}</strong></div>) : <p>팀 편성 뒤 대진이 공개돼요.</p>}</article></section>
  </main>;
}
