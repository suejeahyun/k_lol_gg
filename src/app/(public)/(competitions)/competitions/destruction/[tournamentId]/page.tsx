...
export default async function DestructionDetailPage({ params }: { params: Promise<{ tournamentId: string }> }) {
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.detailGrid}>
      ...
      <article><h2>경기 MVP</h2>{destruction.mvpResults.length ? destruction.mvpResults.map((mvp) => <div className={styles.row} key={mvp.fixtureId}><span>{mvp.fixtureId}</span><strong>{mvp.finalizedPlayerId} · {mvp.selectionMethod}</strong></div>) : <p>완료된 경기의 10인 투표 결과가 여기에 표시돼요.</p>}</article>
    </section>
  </main>;
}
