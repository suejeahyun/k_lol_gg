...
import { competitionBracketStageLabel, competitionPositionLabel } from "@/modules/competitions/core";
...
import { DestructionPublicProgress, DestructionRulesAndStandings } from "@/components/competitions/destruction/public-progress";
...
export default async function DestructionDetailPage({ params, searchParams }: { params: Promise<{ tournamentId: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const ownerResult = session?.accountStatus === "APPROVED" && runtime ? await Promise.all([
    runtime.repository.getOwnApplication(tournamentId, session.userId),
    runtime.repository.getOwnMvpBallots(tournamentId, session.userId),
  ]).then(([own, ownMvpBallots]) => ({ own, ownMvpBallots, available: true })).catch(() => ({ own: null, ownMvpBallots: [], available: false })) : { own: null, ownMvpBallots: [], available: true };
  const { own, ownMvpBallots } = ownerResult;
  ...
  return <div className={styles.page}>
    ...
    <DestructionOwnerActions gameMode={destruction.gameMode} tournamentId={destruction.id} revision={destruction.revision} status={destruction.status} signedIn={Boolean(session)} approved={session?.accountStatus === "APPROVED"} application={own} mvpBallots={ownMvpBallots} available={ownerResult.available} focusTarget={view.action === "apply" ? "apply" : view.tab === "mvp" ? "mvp" : null} />
    {view.tab === "overview" ? <><DestructionRulesAndStandings destruction={destruction} /><Overview destruction={destruction} /></> : null}
    ...
    {view.tab === "participants" ? <section className={styles.tabPanel} aria-labelledby="participants-title"><h2 id="participants-title">참가 선수</h2>{destruction.unassignedPlayers.length ? <article className={styles.team}><h3>팀 배정 대기</h3><ul>{destruction.unassignedPlayers.map((player) => <li key={player.participantId}>{destruction.gameMode === "CLASSIC" ? <span>{competitionPositionLabel(player.position)}</span> : null}<Link href={`?tab=participants&player=${encodeURIComponent(player.playerId)}`}>{player.playerName}</Link></li>)}</ul></article> : null}{selectedPlayer ? <article className={styles.selectedPlayer} tabIndex={-1} autoFocus role="region" aria-label={`${selectedPlayer.playerName} 선수 상세`}><span>{selectedPlayer.teamName}{destruction.gameMode === "CLASSIC" ? ` · ${competitionPositionLabel(selectedPlayer.position)}` : ""}</span><strong>{selectedPlayer.playerName}{selectedPlayer.isCaptain ? " · 주장" : ""}</strong><small>{selectedPlayer.purchasePoints === null ? "경매 대기" : `${selectedPlayer.purchasePoints}P`}</small></article> : null}{destruction.teams.length ? <div className={styles.participantGrid}>{destruction.teams.map((team) => <article className={styles.team} key={team.id}><strong>{team.name}</strong><ul>{team.rosterPlayers.map((player) => <li key={player.participantId}>{destruction.gameMode === "CLASSIC" ? <span>{competitionPositionLabel(player.position)}</span> : null}<Link href={`?tab=participants&player=${encodeURIComponent(player.playerId)}`}>{player.playerName}{player.isCaptain ? " · 주장" : ""}</Link></li>)}</ul></article>)}</div> : !destruction.unassignedPlayers.length ? <p>참가 선수 명단을 준비하고 있어요.</p> : null}</section> : null}
