...
export default async function DisciplinePage() {
  ...
  return <div className={styles.page}>
    ...
    {result.state === "ready" ? <>
      <section className={styles.stats} aria-label="징계 익명 집계"><article className={styles.card} data-severity="caution"><span>활성 주의</span><strong>{result.data.activeCautionCount}</strong></article><article className={styles.card} data-severity="warning"><span>활성 경고</span><strong>{result.data.activeWarningCount}</strong></article><article className={styles.card} data-severity="ban"><span>활성 이용 제한</span><strong>{result.data.activeBanCount}</strong></article><article className={styles.card} data-severity="resolved"><span>해결 완료</span><strong>{result.data.resolvedCount}</strong></article></section>
    ...
    </> : result.state === "unavailable" ? <section className={styles.state} role="status"><h2>징계 현황을 확인할 수 없어요.</h2><p>잠시 후 다시 확인해 주세요.</p></section> : <section className={styles.state} role="alert"><CircleAlert aria-hidden="true" /><h2>징계 집계를 불러오지 못했습니다.</h2><p>잠시 후 다시 시도해 주세요.</p></section>}
  </div>;
}
