...
export async function POST(request: Request, context: Context) {
  ...
  const prepared = await prepareMatchJsonMutation(request, `admin:match-submissions:${submissionId}:reopen`, authorization.session, "ADMIN", 1024);
  if (!prepared.ok) return prepared.response;
  const service = getRuntimeMatchService();
  if (!service) return matchServiceUnavailableResponse(prepared.traceId);
  try {
    return matchMutationResponse(await service.reopenSubmission(prepared.context, submissionId, prepared.expectedRevision, prepared.body), prepared.traceId);
  } catch (error) {
