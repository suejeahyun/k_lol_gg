...
export async function POST(request: Request, context: Context) {
  ...
  const prepared = await prepareSeasonMutation(request, "admin:season-kakao-pending:cancel", authorization.session);
  ...
  try {
    ...
    const result = await service.cancelKakaoPendingApplication(
