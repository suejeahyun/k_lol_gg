...
export async function POST(request: Request, context: { params: Promise<{ draftId: string }> }) {
  ...
  const prepared = await prepareTeamBalanceMutation(request, "team-tools:drafts:archive", auth.session); if (!prepared.ok) return prepared.response;
  const service = getRuntimeTeamBalanceService(); if (!service) return teamBalanceUnavailableResponse(prepared.value.traceId);
  try { return teamBalanceMutationResponse(await service.archiveDraft(prepared.value.context, (await context.params).draftId, prepared.value.expectedRevision, prepared.value.body), prepared.value.traceId); }
