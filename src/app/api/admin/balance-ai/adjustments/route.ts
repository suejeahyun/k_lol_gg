...
export async function POST(request: Request) {
  ...
  const prepared = await prepareMmrMutation(request, auth.session, "admin:mmr:adjust");
  ...
  try {
    return mmrMutationResponse(
      await service.addAdjustment(prepared.value.context, prepared.value.expectedGeneration, prepared.value.body),
