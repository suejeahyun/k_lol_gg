...
export async function GET(request: Request) {
  ...
  try { return mediaReadResponse(await service.listAdminHighlights(query), undefined, traceId); }
  catch (error) { return mediaErrorResponse(error, traceId); }
}

export async function POST(request: Request) {
  const auth = await requireMediaAdminSession(); if (!auth.ok) return auth.response;
  const prepared = await prepareMediaMutation(request, "media:highlights:create", auth.session); if (!prepared.ok) return prepared.response;
  const service = getRuntimeMediaService(); if (!service) return mediaUnavailableResponse(prepared.value.traceId);
  try { return mediaMutationResponse(await service.createHighlight(prepared.value.context, prepared.value.expectedRevision, prepared.value.body), prepared.value.traceId); }
