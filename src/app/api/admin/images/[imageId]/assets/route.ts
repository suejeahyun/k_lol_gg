...
const resource = { resourceType: "GALLERY_ENTRY", purpose: "GALLERY" } as const;

export async function GET(request: Request, context: { params: Promise<{ imageId: string }> }) {
  const auth = await requireMediaAdminSession();
  return auth.ok ? listMediaDraftAssets(request, auth.session, (await context.params).imageId, resource) : auth.response;
}

export async function POST(request: Request, context: { params: Promise<{ imageId: string }> }) {
  const auth = await requireMediaAdminSession();
  return auth.ok ? uploadMediaDraftAsset(request, auth.session, (await context.params).imageId, resource) : auth.response;
}
