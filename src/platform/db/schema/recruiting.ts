...
export const recruitParties = recruitingSchema.table("parties", {
  ...
  check("recruit_parties_organizer_text", sql`${table.organizerText} IS NULL OR char_length(btrim(${table.organizerText})) BETWEEN 1 AND 100`),
...
export const scrimRecruits = recruitingSchema.table("scrims", {
  ...
  check("scrim_recruits_tournament_identity", sql`${table.isDraft} OR ${table.tournamentId} IS NOT NULL OR (${table.legacyTournamentNumber} IS NOT NULL AND ${table.legacyTournamentNumber} BETWEEN 1 AND 9999)`),
  ...
  check("scrim_recruits_requester_identity", sql`${table.isDraft} OR ${table.requesterTeamId} IS NOT NULL OR (${table.requesterTeamName} IS NOT NULL AND char_length(btrim(${table.requesterTeamName})) BETWEEN 1 AND 120)`),
  ...
  check("scrim_recruits_organizer_text", sql`${table.organizerText} IS NULL OR char_length(btrim(${table.organizerText})) BETWEEN 1 AND 100`),
...
export const kakaoBotInstallations = recruitingSchema.table("kakao_bot_installations", {
  ...
  canonicalRoomId: uuid("canonical_room_id").references(() => kakaoRooms.id, { onDelete: "restrict" }),
  ...
  index("kakao_bot_installations_room_idx").on(table.canonicalRoomId, table.status),
